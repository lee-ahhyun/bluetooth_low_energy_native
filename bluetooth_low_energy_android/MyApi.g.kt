  ...
  companion object {
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: MyCentralManagerHostApi?) {
      ...
      run {
        ...
        Log.d("#####stopDiscovery#####","stopDiscovery")
        if (api != null) {
          channel.setMessageHandler { _, reply ->
      ...
      run {
        ...
        Log.d("#####getMWLength","getMaximumWriteLength")
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val peripheralHashCodeArgsArg = args[0].let { if (it is Int) it.toLong() else it as Long }
      ...
      run {
        Log.d("#####discoverGATT#####","discoverGATT")
      ...
      run {
        Log.d("#####readCharacteristic","readCharacteristic")
      ...
      run {
        Log.d("#####notifyCharacteristic","notifyCharacteristic")
      ...
      run {
        Log.d("#####readDescriptor","readDescriptor")
      ...
      run {
        Log.d("#####writeDescriptor","writeDescriptor")
...
class MyCentralManagerFlutterApi(private val binaryMessenger: BinaryMessenger) {
  ...
  fun onDiscovered(peripheralArgsArg: MyPeripheralArgs, rssiArgsArg: Long, advertisementArgsArg: MyAdvertisementArgs, callback: (Result<Unit>) -> Unit) {
    ...
    // Log.d("onDiscovered","@@@@@@@@@@@@"+ peripheralArgsArg + "advertisementArgsArg" +advertisementArgsArg )
      if (it is List<*>) {
        if (it.size > 1) {
          callback(Result.failure(FlutterError(it[0] as String, it[1] as String, it[2] as String?)));
        } else {
